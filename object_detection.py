...
def write(bboxes, img, classes, colors):
    ...
    feedback = (
        "{}".format(labels["Current Object"])
        + " "
        + "is"
        + " at {} ".format(round(distance))
        + "Inches"
    )
    ...
    cv2.rectangle(
        img, (bboxes[0], bboxes[1]), (bboxes[2] + text_w - 30, bboxes[3]), color, 2
    )
    cv2.putText(
        img,
        text_str,
    ...
    def main(self):
        ...
        while True:
            ...
            try:
                ...
                if (
                    self.CUDA
                ):  #### If you have a gpu properly installed then it will run on the gpu
                    im_dim = im_dim.cuda()
                    img = img.cuda()
                ...
                if list(output.size()) == [1, 86]:
                    ...
                    output[:, 1:5] = (
                        torch.clamp(output[:, 1:5], 0.0, float(self.inp_dim))
                        / self.inp_dim
                    )
                    ...
                    list(
                        map(
                            lambda boxes: write(
                                boxes, frame, self.classes, self.colors
                            ),
                            output,
                        )
                    )
